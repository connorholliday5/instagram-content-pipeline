...
def _stars(canvas: Image.Image):
    ...
    for _ in range(280):
        ...
        draw.ellipse([(x-r,y-r),(x+r,y+r)],
                     fill=(180+rng.randint(0,55), 200+rng.randint(0,45), 230+rng.randint(0,25), a))
...
def _base() -> Image.Image:
    ...
    d = ImageDraw.Draw(ov)
    d.ellipse([(-200,-200),(W+200,600)], fill=(0,0,30,50))
    d.ellipse([(-200,750),(W+200,H+200)], fill=(0,0,40,60))
...
def _place_poster(canvas: Image.Image, url: Optional[str],
    ...
    if img:
        ...
        ph = Image.new("RGB", (iw, ih), (10, 12, 22))
...
def _pill_overlay(canvas: Image.Image, box: tuple, line1: str,
    ...
    b2 = draw.textbbox((0,0), line2, font=f2) if line2 else (0,0,0,0)
    tw = max(b1[2]-b1[0], b2[2]-b2[0])

    strip_h = 28 + (26 if line2 else 0) + 24
    strip_y = y2 - strip_h - 6

    ov = Image.new("RGBA", SIZE, (0,0,0,0))
    od = ImageDraw.Draw(ov)
    od.rectangle([(x1+4, strip_y),(x2-4, y2-4)], fill=(0,0,0,190))
    canvas.alpha_composite(ov)
    ...
    od2.rectangle([(px1, pill_y),(px2, pill_y+28)],
                  fill=color+(50,), outline=color+(220,), width=2)
    canvas.alpha_composite(ov2)

    draw = ImageDraw.Draw(canvas)
    draw.text((cx, pill_y+14), line1, font=f1, fill=color+(240,), anchor="mm")
    if line2:
        draw.text((cx, pill_y+42), line2, font=f2,
...
def _draw_station(canvas: Image.Image, top_y: int, scale: float = 0.88):
    ...
    def ring(gy,gr,grv,width,alpha_fill=25):
        ...
        draw.ellipse([(cx-gr+9,gy-grv+4),(cx+gr-9,gy+grv-4)],
                     outline=(0,80,150,140),width=2)
        ov=Image.new("RGBA",SIZE,(0,0,0,0)); od=ImageDraw.Draw(ov)
        od.ellipse([(cx-gr,gy-grv),(cx+gr,gy+grv)],fill=(0,140,200,alpha_fill))
        canvas.alpha_composite(ov)
        d=ImageDraw.Draw(canvas)
    ...
    for wy in [top_y+s(248),top_y+s(263),top_y+s(278),top_y+s(293)]:
        ...
        for i in range(5):
            ...
            draw.rectangle([(wx,wy+1),(wx+s(34),wy+s(7))],
                           fill=(20,48,108) if lit else PANEL)
    for sign in [-1,1]:
...
# SLIDE 1 — COVER
# ─────────────────────────────────────────────────────────
def build_tv_cover(month_date: date) -> Path:
    canvas = _base()
    station_top = 35
    ...
    draw.text((W//2, block_top+line_h//2), "TV SHOWS",
    ...
    return _save(canvas, "tv_01_cover")


# ─────────────────────────────────────────────────────────
# SLIDE 2 — TOP 10 PREMIERING
# ─────────────────────────────────────────────────────────
def build_tv_top10(shows: list, month_date: date,
                   get_title, get_poster_url) -> Path:
    canvas = _base()
    header_bottom = _header(canvas, "TOP 10 SHOWS THIS MONTH")

    pad = 10
    gt = header_bottom + 14
    gb = H - 78

    # 4 cols x 3 rows portrait grid
    cols, rows = 4, 3
    ...
    for idx in range(10):
        ...
        row = idx // cols
        if row == 2:
            items_last = 10 - cols*(rows-1)
            total_w = items_last*cw + (items_last-1)*pad
            start_x = (W - total_w) // 2
...
# SLIDE 3 — YOUR PICKS
# ─────────────────────────────────────────────────────────
def build_tv_picks(picks: list, month_date: date,
                   get_title, get_poster_url) -> Path:
    canvas = _base()
    ...
    if n == 0:
        ...
        cw = (W - pad*3) // 2
        ch = gb - gt
        for idx, show in enumerate(picks):
            x1 = pad + idx*(cw+pad)
            _place_poster(canvas, get_poster_url(show),
                         (x1, gt, x1+cw, gt+ch),
                         ACCENT, idx+1, get_title(show))
    ...
    _footer(canvas, month_date.strftime("%B %Y").upper())
    return _save(canvas, "tv_03_picks")


# ─────────────────────────────────────────────────────────
# SLIDE 4 — MOST POPULAR LAST MONTH
# ─────────────────────────────────────────────────────────
def build_tv_popular(shows: list, month_date: date,
                     get_title, get_poster_url, get_network,
                     get_rating, get_vote_count, format_votes) -> Path:
    canvas = _base()
    ...
    header_bottom = _header(canvas, "MOST POPULAR LAST MONTH")
    draw = ImageDraw.Draw(canvas)
    ...
    # Source note
    src_f = _font("regular", 20)
    draw.text((W//2, header_bottom+50), "SOURCE: TMDB COMMUNITY RATINGS",
              font=src_f, fill=GRAY_DIM+(120,), anchor="mm")
    ...
    for idx, show in enumerate(shows):
        ...
        _place_poster(canvas, get_poster_url(show),
                     (x1, gt, x1+cw, gt+ch),
                     border_color, idx+1, get_title(show))

        # Show rating + vote count as the metric
        rating = get_rating(show)
        votes = get_vote_count(show)
        line1 = f"★ {rating}/10" if rating > 0 else "NO RATING YET"
        line2 = format_votes(votes) if votes > 0 else ""

        _pill_overlay(canvas,
                     (x1, gt, x1+cw, gt+ch),
                     line1, line2,
                     border_color)

    _footer(canvas, month_date.strftime("%B %Y").upper())
    return _save(canvas, "tv_04_popular")